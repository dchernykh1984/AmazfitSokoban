...
// A row of `count` equal boxes side by side inside the same safe chord, with a
// gap between them - the bottom cap of a round screen is only wide enough for two
// small buttons, and they still have to stay off the bezel.
export function splitRow(box, count, gap) {
  const columns = Math.max(1, Math.floor(count));
  const spacing = Math.max(0, gap);
  const width = Math.floor((box.w - spacing * (columns - 1)) / columns);
  const boxes = [];
  for (let i = 0; i < columns; i++) {
    boxes.push({ x: box.x + i * (width + spacing), y: box.y, w: width, h: box.h });
  }
  return boxes;
}
