...
// be half on screen, so the range is deliberately generous at both ends and the
// canvas clips what hangs over.
export function visibleCells(camera) {
  const first = Math.floor(camera.x / camera.cell);
  const last = Math.ceil((camera.x + camera.visible * camera.cell) / camera.cell);
  const firstRow = Math.floor(camera.y / camera.cell);
  const lastRow = Math.ceil((camera.y + camera.visible * camera.cell) / camera.cell);
  return {
