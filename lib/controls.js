...
const COUNTER_FILL = 0.35;
const ARROW_FILL = 0.5;

// How far down the top segment the counters start. Not at the very top: a round
// screen is barely 70px across up there, and "0/7   123" needs 110. A few pixels
// lower the chord has opened up enough to hold the line, and the up arrow still
// has its full height underneath.
const COUNTER_TOP = 0.14;
...
// as wide as the chord at its lowest edge, which is 131px on a 466 screen and
// 134px on a 480 - three buttons in that are shoulder to shoulder. Moved up
// against the board it gets over 200px to share, and that width buys the gaps.
//
...
export function controlLayout(screenSize, board) {
  ...
  const counterTop = Math.round(top * COUNTER_TOP);
