...
import { centeredBox, splitRow } from "./round-geometry.js";
...
export function controlLayout(screenSize, board) {
  ...
  const upHeight = Math.round(top * SEGMENT_FILL);
  const upTop = Math.round((top - upHeight) / 2);
  ...
  const rowTop = board.y + board.size + Math.round((bottom - rowHeight) / 2);
  const row = centeredBox(screenSize, rowTop, rowHeight, board.size, EDGE_PADDING);
  const cells = splitRow(row, 3, Math.round(row.w * ROW_GAP));
