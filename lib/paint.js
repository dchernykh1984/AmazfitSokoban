...
export function paintArrow(direction, box, color) {
  ...
  // The buttons are nowhere near square - the up arrow sits in a wide, shallow
  // strip and the side arrows in tall, narrow ones - so the chevron is sized
  // from the SHORT side and only allowed to stretch half as much again. Sizing
  // each reach off its own side instead would draw the up arrow 72px wide and
  // 14px tall: at that angle two strokes read as a bent line, not as an arrow,
  // and the four arrows would not even look like the same symbol.
  const base = Math.min(box.w, box.h) * 0.22;
  const reachX = Math.min(box.w * 0.22, base * 1.5);
  const reachY = Math.min(box.h * 0.22, base * 1.5);
  const width = Math.max(3, Math.round(Math.min(box.w, box.h) * 0.12));

  let tip;
  let armA;
  let armB;
  if (direction === UP) {
    tip = [midX, midY - reachY];
    armA = [midX - reachX, midY + reachY];
    armB = [midX + reachX, midY + reachY];
  } else if (direction === DOWN) {
    tip = [midX, midY + reachY];
    armA = [midX - reachX, midY - reachY];
    armB = [midX + reachX, midY - reachY];
  } else if (direction === LEFT) {
    tip = [midX - reachX, midY];
    armA = [midX + reachX, midY - reachY];
    armB = [midX + reachX, midY + reachY];
  } else {
    tip = [midX + reachX, midY];
