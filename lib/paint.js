...
// stroke included. That is the up arrow's height - the top strip is the tightest
// of the four - so the fraction is set as high as that strip will take rather
// than left at something comfortable: these are the controls the whole game is
// played with, and a 24px chevron in a 67px button reads as an afterthought.
export function arrowMetrics(boxes) {
  ...
  const reach = Math.max(0, Math.floor(Math.min(shortest * 0.42, room - width / 2)));
  return { reach, width };
}
