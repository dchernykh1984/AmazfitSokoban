...
export function paintArrow(direction, box, color) {
  ...
  const reachX = box.w * 0.22;
  const reachY = box.h * 0.22;
  ...
  if (direction === UP) {
    ...
    left = [midX - reachX, midY + reachY];
    right = [midX + reachX, midY + reachY];
  ...
  return [thickLine(left, tip, width, color), thickLine(tip, right, width, color)];
}
