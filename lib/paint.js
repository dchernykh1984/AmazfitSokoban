...
export function arrowMetrics(boxes) {
  ...
  // Half the stroke sits outside the endpoints, so the reach has to leave room
  // for it or the arrow overhangs the button it is drawn in.
  const reach = Math.max(1, Math.min(shortest * 0.34, shortest / 2 - width / 2 - 1));
  return { reach, width };
}
